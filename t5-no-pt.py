...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, train_data=None, val_data=None, file_type="train"):
        if file_type == "train":
            sources = train_data["function_before_tokenized"].tolist()
            labels = train_data["function_after_tokenized"].tolist()
        elif file_type == "eval":
            sources = val_data["function_before_tokenized"].tolist()
            labels = val_data["function_after_tokenized"].tolist()
        elif file_type == "test":
            # data = datasets.load_dataset("MickyMike/cvefixes_bigvul", split="test")
            data = pd.read_csv('data/test.csv')
            sources = data["function_before_tokenized"]
            labels = data["function_after_tokenized"]
        self.examples = []
...
def convert_examples_to_features(source, label, tokenizer, args):
    ...
    label = tokenizer.encode(label, truncation=True, max_length=args.decoder_block_size,
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    for i in range(0, len(generated_texts), args.num_beams):
        ...
        ground_truth = clean_tokens(tokenizer.decode(
            groundtruth_sentence[counter], skip_special_tokens=False))
...
# truth
    ...
    df.to_csv(
        "./predictions/no-pretraining/predictions-beam-{}.csv".format(args.num_beams))
